...
% Encode 'description' column into numerical values
[uniqueDescriptions, ~, encodedValues] = unique(transactions.description);

% Add the encoded values as a new column
transactions.category_encoded = encodedValues;

% Display a confirmation message
disp('Categories encoded successfully into "category_encoded".');

% Encode 'use_chip' Feature
% Define the mapping: 'Swipe' -> 0, 'Onlin' -> 1
chipMapping = containers.Map({'Swipe', 'Onlin'}, [0, 1]);

% Apply the mapping to convert the column
transactions.use_chip = cellfun(@(x) chipMapping(x), transactions.use_chip);

% Verify the conversion
disp('Unique values in the encoded "use_chip" feature:');
disp(unique(transactions.use_chip)); % Should display [0; 1]
